...
def plots(
    ...
    plot1 = plt.figure()
    axes[1, 0].imshow(
        post_mean_numpy / np.sqrt(post_meanvar.get_var().detach().cpu().numpy()),
        cmap="gray",
    )
    ...
    plot1 = plt.figure()
    axes[1, 2].imshow(
        torch.log(post_meanvar_absfourier.get_mean()).detach().cpu().numpy()
    )
    ...
    plot1 = plt.figure()
    axes[1, 3].imshow(
        torch.log(post_meanvar_absfourier.get_var()).detach().cpu().numpy()
    )
    ...
    plot1 = plt.figure()
    axes[0].plot(
        np.arange(len(nrmse_values))[::10], nrmse_values[::10], label="-- NRMSE --"
    )
    ...
    plot1 = plt.figure()
    axes[1].plot(
        np.arange(len(psnr_values))[::10], psnr_values[::10], label="-- PSNR --"
    )
    ...
    plot1 = plt.figure()
    axes[2].plot(
        np.arange(len(ssim_values))[::10], ssim_values[::10], label="-- SSIM --"
    )
    ...
    plt.plot(
        np.arange(len(logPi_trace))[::10], logPi_trace[::10], label="- $\log \pi$ -"
    )
    plt.legend()
    plt.show()
    plt.close()
...
def downsampling_variance(X_chain):
    ...
    for k, i in enumerate(scale):
        ...
        for j in range(n_samples):
            downsample_array[j] = block_reduce(
                X_chain[j], block_size=(i * 2, i * 2), func=np.mean
            )
