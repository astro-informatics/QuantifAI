...
class FastPixelUQ(object):
    ...
    def compute_HPD_region(self):
        ...
        if self.potential_handle is not None:
            ...
            self.gamma_alpha = (
                self.potential_handle(self.x_map).item()
                + self.tau_alpha * np.sqrt(self.N)
                + self.N
            )
        else:
            raise ValueError(
                "The potential_handle has not been defined. Set it manually in the initialisation or call self.set_quantifai_potential_fun_handle(...)."
            )
    ...
    def compute_HPD_region_threshold(self):
        ...
        thresh_img = self.oper2wavelet.full_op_threshold_img(torch_map, selected_thresh)
    ...
    def compute_thresh_imgs_per_scale(self, x_gt=None, verbose=False):
        ...
        for modif_level in range(self.UQ_options["levels"] + 1):
            ...
            if x_gt is not None:
                ...
                if verbose:
                    print(
                        "SNR (thresh vs MAP) at lvl {:d}: {:f}".format(
                            modif_level,
                            eval_snr(to_numpy(torch_map), to_numpy(modif_img)),
                        )
                    )
                    print(
                ...
                SNR_at_lvl_list.append(eval_snr(x_gt, to_numpy(modif_img)))
                SNR_at_lvl_map_vs_GT_list.append(eval_snr(x_gt, to_numpy(GT_modif_img)))
