...
for it in range(options['iter']):
    ...
    x_hat = z - alpha *(
        torch.tensor(g.grad(z.detach().cpu().squeeze()), device=device, dtype=myType).reshape((1,1) + x_init.shape) + lmbd * model(mu * z)
    )
...
def _reg_fun(x_hat, model, mu, lmbd):
    return (lmbd/mu) * model.cost(
        mu * torch.tensor(x_hat, device=device, dtype=myType, requires_grad=False).reshape((1,1) + x_hat.shape)
