...
plt.figure(figsize=(10,12))
axs = plt.gca()
axs.plot(lmbd_list[:zoom_id], np.array(likelihood_list)[:zoom_id], '-o', label='f(x_map)')
axs.plot(lmbd_list[:zoom_id], np.array(prior_list)[:zoom_id], '-o', label='g(x_map)')
axs.plot(lmbd_list[:zoom_id], np.array(const_gamma_alpha_list)[:zoom_id], '-o', label='tau_alpha')
axs.legend(fontsize=18, loc='upper center')
axs.set_ylabel(r'Potentials')
axs.set_xlabel(r'Reg strength')
ax2 = axs.twinx()
...
axs = plt.gca()
for it in range(len(superpix_sizes)):
    axs.plot(lmbd_list[:zoom_id], mean_LCI[it,:zoom_id], '-o', label='Pix size %d'%(superpix_sizes[it]))
axs.legend(fontsize=18, loc='upper center')
axs.set_ylabel(r'Potentials')
axs.set_xlabel(r'Reg strength')
ax2 = axs.twinx()
...
for it in range(len(superpix_sizes)):
    axs[0,1].plot(lmbd_list[:zoom_id], mean_LCI[it,:zoom_id], '-o', label='Pix size %d'%(superpix_sizes[it]))
...
axs[1,1].plot(lmbd_list[:zoom_id], np.array(likelihood_list)[:zoom_id], '-o', label='likelihood(x_map)')
axs[1,1].plot(lmbd_list[:zoom_id], np.array(prior_list)[:zoom_id], '-o', label='prior(x_map)')
axs[1,1].plot(lmbd_list[:zoom_id], np.array(const_gamma_alpha_list)[:zoom_id], '-o', label='tau_alpha')
